...
sim=function(n=30,mu=0,sigma=5){
     ...
     test_statistic=t.test(data,mu=0,conf.int = 0.95)
     data %>% 
       summarize(
           mu_hat=pull(broom::tidy(test_statistic), estimate),
           p_value=pull(broom::tidy(test_statistic), p.value)
    )
}
```
...
sim_result= 
  tibble(mean= c(0:6)) %>% 
  mutate(
    output=map(.x=mean, ~rerun(5000, sim(mu = .x))),
    estimate_df=map(output,bind_rows)
    ) %>% 
  select(-output) %>% 
  unnest(estimate_df)
```
